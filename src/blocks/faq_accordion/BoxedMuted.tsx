...
const BoxedMuted: BlockComponent<FaqAccordionContent> = ({ items }) => (
      ...
      <Accordion type="single" collapsible className="flex flex-col gap-3">
        {items.map((item, i) => (
          <AccordionItem
            key={i}
            value={String(i)}
            className="rounded-xl border border-border bg-card text-card-foreground shadow-sm ring-1 ring-transparent transition-all data-[state=open]:shadow-md data-[state=open]:ring-primary/20"
          >
